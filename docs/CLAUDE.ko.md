...
**도구 규율 (Google/Stripe/Anthropic 엔지니어링 관행 기반):**
- 기존 파일 수정 시: `Edit` 도구로 타겟 블록만 변경 — `Write`로 파일 전체를 덮어쓰지 마라
- 수정 전 정확한 스코프 선언: "`[file:line_start–line_end]`의 `[function]` 변경"
- 하나의 작업에서 3개 이상의 파일을 건드려야 한다면 작업을 분리하라
...
| 버그 수정 | 변경 라인 30줄 미만 |
| 기능 추가 | 변경 라인 150줄 미만 |
| 리팩터링 (요청 시) | 변경 라인 200줄 미만 |

예산 초과가 예상되면 일단 멈추고 사용자에게 작업 분리 여부를 물어라.

**금지된 diff 노이즈:**
- 건드리지 않은 코드 재포맷 (들여쓰기, 따옴표, import 순서)
- 작업과 무관한 변수/함수 이름 변경
- 로직 변경 없는 코드 블록 이동
