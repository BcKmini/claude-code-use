...
새 PC·계정에서 이 레포를 그대로 재현할 때 순서대로 따라가면 됩니다.
...
| GNU make | — | `make --version` *(Makefile 사용 시 필요)* |
...
두 방법 모두 `/snippet`, `/handoff`, `/cost`, `/review-diff`, `/remind` 슬래시 커맨드를 `~/.claude/commands/`에 설치하고, 기본 스니펫 20개를 임포트합니다.
...
전체 Rust 워크플로우 → [INTEGRATION.ko.md](INTEGRATION.ko.md)
