...
라이브러리·프레임워크 최신 문서를 실시간으로 조회. 별도 설치 불필요 — Claude Code에서 라이브러리 관련 질문 시 자동 활성화.
...
설치 내용: `/snippet`, `/handoff`, `/cost` 슬래시 커맨드 + 쉘 함수 + 기본 스니펫 20개.
...
[ ] 12. 도구 설치: bash tools/install-tools.sh
[ ] 13. (선택) claw-code 빌드 → claw doctor 헬스체크
```
