...
| 시스템 | 역할 | 링크 |
|--------|------|------|
| Claudecode-Agent | 에이전트 정의 (9개 에이전트 역할·동작 규칙) + CLI 도구 | 이 레포 |
...
claw-code는 `~/.claude/agents/`가 아닌 자체 에이전트 시스템을 사용한다. 하지만 **orchestrator 패턴**은 claw-code에서도 직접 활용 가능하다:
...
이후 claw 세션에서 코드베이스 관련 질문 시 RAG가 관련 코드를 자동으로 참조.
